...
- 2026-09-24 确认：`1.0.0-beta.1` 封版，Play 封闭测试使用的版本不做任何修改；真机界面审查发现的问题全部在正式上架前处理，见[正式上架前待办](LAUNCH_BACKLOG.md)和 GitHub 里程碑"正式上架前"（#32–#54）。唯一例外是应用内隐私政策（#36）：Google Play 要求所有应用在应用内提供隐私政策，用户同日确认加入 beta.1。About 页提供中英文全文，文本在构建时从 `docs/PRIVACY_POLICY*.md` 打包，因此这两个文件必须保持在解析器支持的格式内（有测试校验），同时提供在浏览器中打开网页版的链接。
...
4. Application signing、Play upload 与离线 verification issuer 已分别创建并完成本机 Keychain 回读、开库和私钥使用验证；私钥、alias 对应密码均留在仓库外。最终发布前仍须完成加密离线备份、Play App Signing 导入与证书对账，生成并核验签名 APK/AAB，在安装候选上核验数据保留和无诊断入口，再交付两个渠道。Play 账号准入已于 2026-09-22 确认；商业模式于 2026-09-23 改为免费下载加应用内买断，见下节。

第二款 DAC 不再是本次受限 Beta 的阻断项；崩溃/ANR、数据丢失、错误输出声明、签名/安装升级失败及 Release 诊断开关仍阻断发布。

## Google Play 上架与封闭测试

2026-09-23 起，主策略为免费下载 + 应用内试用期 + 到期后一次性应用内买断。完整说明和已核实的 Play 约束见[商业模式](MONETIZATION.md)。它取代 2026-09-22 记录的"先设价、低价测试再退款"路线。

- **beta.1 免费上架，不含收费功能。** `1.0.0-beta.1` 在 GitHub Releases 和 Play 封闭测试中都是免费的完整版本，不加入 Billing 库、试用或付费墙；试用和应用内购买放到之后的版本。应用在 Play 上永久免费是已确认的单向决定：免费提供过的应用不能再改为付费下载。
- **暂不建立付款资料。** 应用内购买同样会让账号成为 merchant account，需要公开付款资料中的完整地址。账号主体（个人或组织）确定之前，不建立付款资料。
- **封闭测试。** 发布账号是约 2026-08 注册的个人 Play Console 账号，适用[新个人开发者账号的测试要求](https://support.google.com/googleplay/android-developer/answer/14151465)：上架正式版前，至少 12 名测试者连续加入至少 14 天，之后单独申请正式版权限。应用免费，测试者直接安装；收费版本上线时，可以用促销码向参与者赠送解锁。
- **测试者范围。** 测试者需要 Google 账号，并位于封闭测试开放的国家或地区；只有内部测试不受地区限制。中国大陆无法使用 Google Play。招募人数高于 12 人，避免中途有人退出后连续满 14 天的人数不足。
- 仓库只记录测试人数、起止日期和结论，不记录测试者身份、邮箱或订单号。
...
- [ ] Privacy：最终合并 Manifest 不含 INTERNET（适用于不含 Billing 的构建；含 Billing 的构建见[商业模式](MONETIZATION.md)）；如依赖声明 ACCESS_NETWORK_STATE，准确说明它只用于查询网络连接状态。无后台上传新增。Release 不提供诊断启用、录制或导出入口；Debug/Internal 导出仍须显式操作并按策略脱敏。
...
- [x] 商业模式已确认（2026-09-23）：免费下载 + 应用内试用 + 一次性应用内买断；应用在 Play 上永久免费是已知晓的单向决定。
- [ ] `1.0.0-beta.1` 以免费应用上传；Release 合并 Manifest 不含 Billing 库、`com.android.vending.BILLING` 或付费墙代码。
- [ ] 封闭测试至少 12 名测试者连续加入满 14 天并获得正式版权限；测试人数和起止日期已记录。
- [ ] `./gradlew :app:checkPrivacyPolicyFinal` 通过：应用内隐私政策为定稿全文，中英文网页地址已填写（#36、#38、#43）。`bundleRelease` 会自动执行这项检查；GitHub APK 发布前需要手动执行。
