...
- 2026-09-24 确认：`1.0.0-beta.1` 封版，Play 封闭测试使用的版本不做任何修改；真机界面审查发现的问题全部在正式上架前处理，见[正式上架前待办](LAUNCH_BACKLOG.md)和 GitHub 里程碑"正式上架前"（#32–#54）。其中应用内隐私政策（#36）是 Google Play 对所有应用的政策要求，封闭测试版本同样经过审核，缺少它存在被拒或收到政策警告的风险。
...
- [ ] 上传 Play 封闭测试前，确认是否接受 `1.0.0-beta.1` 缺少应用内隐私政策的审核风险（#36）；隐私政策网址已可以公开访问（#43）。
