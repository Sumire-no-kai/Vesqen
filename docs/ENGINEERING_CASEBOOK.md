...
## P05 · 播放事件触发整库更新的偶发卡顿路径（2026-09-16）

**现象与代码链。** 播放开始后，应用级 history recorder 原来只向 UI 发送 track ID。`VesqenViewModel` 收到后调用 `loadCachedLibrary()`，从 SQLite 重新读取全部曲目、来源和所有歌单，再把整份曲库同步给 `PlaybackController`。这不是持续滚动每一帧都会发生的工作，但它会与用户刚点播后继续上下滑动重叠；曲库越大、歌单越多，尾部延迟越容易表现为偶发小顿。旧 `readPlaylists()` 还会先查歌单、再为每个歌单各查一次成员，形成 1+N 查询。

**修复。** history backend 现于同一 SQLite transaction 中完成计数更新并读回权威的 `playCount / lastPlayedAtMs`，通过有界非阻塞事件只替换 UI 中对应的单曲。若一次较早开始的 catalog 读取稍后返回，ViewModel 会叠加最新历史值，直到数据库快照证明已包含同等或更新的数据，避免计数回滚。歌单快照改为一次读歌单、一次按歌单/位置读全部成员。Now 的 AnimatedContent key 只包含曲目及封面身份，收藏或历史字段变化不再重播整套曲目切换动画。

**为什么没有直接改 CPU 调度。** 设备调度器、RenderThread 和 OEM 刷新策略不是应用可以可靠强制的接口；提高线程优先级也可能挤压音频或系统工作。本次先移除可证明的应用侧全量 IO、对象替换和无关动画，同时保持数据库、队列和播放线程所有权不变。

**验证与边界。** 单元回归覆盖写入失败隔离、有界背压、原子返回值、单行投影及较早快照不得回滚；iQOO 定向 instrumentation 覆盖存储、迁移和新增 UI 语义。没有为 P05 单独取得同一最终 Profile APK 的修复前后帧统计，因此不能把它写成 M3-R1 性能验收完成，也不能将所有剩余慢帧归因于这一路径。Honor 配对、高刷新率和 1k/10k 曲库仍按 M3 门禁执行。
