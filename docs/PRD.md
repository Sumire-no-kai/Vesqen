...
- 完整播放页提供大封面、可展开标题/艺术家、事实性输出状态、进度和核心播放控制；曲目详情与播放链证据分别通过明确入口打开，不在主控制区堆叠参数。
- 完整播放页在紧凑窗口中是沉浸式焦点面：隐藏底部顶层导航以保留封面与控制空间；工具栏和 Android Back 都返回其来源（通常为 Library），状态 chip 仍可进入 Chain，不允许形成回退到桌面的死路。
- 完整播放页不是纵向信息流：曲名始终为较小的单行，超长名称以横向跑马灯呈现而不换行；页面按可用高度和字体缩放收缩封面与间距，绝不通过竖向滚动或隐藏基础播放控制来容纳内容。极端字体缩放时，次要艺术家/route chip 可移至既有的横向会话页，仍保持可达。
...
- Now 的紧凑焦点面是上述导航规则的空间例外：底部导航在其中不常驻，但显式 Back、系统 Back 和 Chain 入口始终可达；返回后恢复稳定的 `Library / Now / Chain` 导航。
