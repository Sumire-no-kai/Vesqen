...
- 有活动播放时，Now 可作为 C 的受保护 Midnight Violet 焦点面覆盖外层深浅主题：其根 Surface 必须显式设置高对比前景色，并接管状态栏/导航栏的深色图标策略；在宽窗口中 rail 也必须让位，避免透明系统栏同时跨越深浅背景；退出 Now 后恢复外层系统栏外观。该例外不改变 Library / Now / Chain 的信息架构。
