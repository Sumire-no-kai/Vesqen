...
- 普通模式支持系统音量。
- 睡眠定时器作为稳定版后续的小型功能。
- 曲库底部提供持久 mini-player，包含封面、标题/艺术家、上一首、播放/暂停和下一首；非按钮区域打开完整播放页。
- 完整播放页采用“封面舞台 + 单一底部运输台”：上部是大封面，下部不透明运输台承载固定单行曲名、空间允许时的艺术家/专辑、事实性输出状态、进度和控制；曲目详情与播放链证据分别通过明确入口打开，不在主控制区堆叠参数。
- 上一首／播放暂停／下一首必须组成居中的主控制组；随机、循环、分页点与圆形 `i` 为底栏次级控制，不能与三枚运输控制同排抢占层级。`SYSTEM MIXED` 只在 Now/Chain 的事实入口出现，不在 mini-player 中挤占标题空间。
...
- 完整播放页不是纵向信息流：曲名与页头始终为较小的单行，超长曲名以横向跑马灯呈现而不换行；页面按可用高度和字体缩放收缩封面与间距，绝不通过竖向滚动或隐藏基础播放控制来容纳内容。极端字体缩放时，次要艺术家/route chip 可移至既有的横向会话页，仍保持可达。
- 完整播放页提供可横向切换的播放会话信息层，仅展示当前可证明的播放/暂停、进度、剩余时间、队列位置和 `SYSTEM MIXED` 声明；不得伪造实时编码、采样率、PCM 或 bit-perfect 指标。
- mini-player 展开为完整播放页时使用约 240 ms 的空间连续转换；系统开启减少动效时改为约 80 ms 的交叉淡入淡出。
- 循环只保留一个熟悉的 48 dp 次级入口，按“关闭 → 列表循环 → 单曲循环 → 关闭”轮换；单曲循环使用带 `1` 的标准图标，而不是新增独立按钮。关闭态为中性弱化色，两个启用态使用品牌主色，并提供准确的 TalkBack 状态。
...
- 有活动播放时，Now 可作为 C 的受保护 Midnight Violet 焦点面覆盖外层深浅主题：其根 Surface 必须显式设置高对比前景色；状态栏背景延续 Midnight artwork field，导航栏背景延续底部 transport dock，并关闭 OEM 自动对比衬底。浅色系统图标/手势标记保留在深色背景上以满足可读性。宽窗口中 rail 也必须让位，避免系统栏同时跨越深浅背景；退出 Now 后完整恢复外层系统栏状态。该例外不改变 Library / Now / Chain 的信息架构。
...
- 紧凑窗口使用带文字标签的底部导航；中等和展开窗口使用同顺序的 navigation rail。
...
- mini-player 位于顶层导航之上；点击其非控制区域进入 Now。Chain 状态 chip 可从 mini-player 或 Now 提供直达，但不能代替永久导航。
