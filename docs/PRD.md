...
开发与内部测试构建可以导出一份不含音乐内容和个人路径的诊断报告。公开 Release
构建必须在构建期关闭诊断录制，不显示启用、录制或导出入口，也不能通过持久化偏好、
备份恢复、Deep Link 或 ADB 设置重新启用。Release 关闭时不得启动诊断采样、创建诊断
文件或执行诊断落盘；Android、Media3 或设备系统自身可能产生的系统日志不属于 Vesqen
可控制或读取的应用诊断日志。
...
- 仅 Debug/Internal 构建支持开始/停止本地诊断录制，开发者明确操作后才持续采样；录制
  使用应用私有、有限容量的持久化存储，进程异常终止后保留可恢复记录，导出前进行隐私
  清理。Profile 与公开 Release 默认且强制关闭，界面不提供开关或入口。
...
- 紧凑窗口使用带文字标签的 60 dp 底部导航（系统 navigation inset 额外保留），文字基线向上校正 4 dp 以保留底部呼吸空间，每个目的地仍提供至少 48 dp 触控目标；其颜色与曲库画布连续，标签单行且可省略，不能裁切大字号。选择目的地时以 180 ms 内的色彩与轻微图标缩放确认状态，减少动效时只保留 80 ms 淡入淡出。中等和展开窗口使用同顺序的 navigation rail。
...
- Library 内的收藏与集合详情使用同一套短距离水平层级转场，返回时反向播放；转场只表达进入或退出层级，不为普通列表刷新增加动画。减少动效时退化为 80 ms 交叉淡入淡出。
- Settings 的菜单树固定为“播放输出 / 音频证据 / 应用信息”：系统输出与严格 USB 是同一互斥模式组；播放链路和签名验证记录归入证据工具；版本与关于归入应用信息。不得把验证记录伪装为输出模式，也不得为视觉充实增加尚未实现的设置项。
...
4. Now 显示标题、艺术家、封面、进度、核心控制和事实性输出状态；源文件参数进入曲目详情，完整输出证据进入 Chain。
   播放器顶部 USB 入口提供严格 USB bit-perfect 开关，与 Settings 共享服务侧模式；开关开启不等于已生效或通过外部验证。严格输出失败时，无论操作来自曲库、mini-player 还是 Now，都弹出具体原因，允许保留严格模式或明确切换系统输出，禁止静默降级。切换系统输出后由用户再次点击播放。
...
2026-09-10 受限 Beta 发布决定：用户确认先向 GitHub Releases 和 Google Play 测试渠道
交付受限 Beta。第二款 DAC 暂时不可用，双手机/双 DAC 完整矩阵及外部数字验证缺失
作为公开限制保留，不阻挡本次受限 Beta，也不据此宣布 M3/M4 完整验收或提升输出声明。
先完成可用设备的适用回归、USB 短测和发布材料；长期发布签名在最终发布阶段创建。
崩溃、数据丢失、错误 ACTIVE/VERIFIED、签名/安装升级问题和 Release 诊断关闭要求
仍是发布门禁。长时间测试继续按用户安排暂缓。具体检查表见 [M4 Beta 发布清单](M4_BETA_RELEASE.md)。
...
- Vesqen 采用免费下载 + 应用内试用期 + 到期后一次性应用内买断（Google Play 一次性商品），不做订阅、广告、Vesqen 账号或服务器。策略、已核实的平台约束和待决定事项见[商业模式](MONETIZATION.md)。
- 试用和应用内解锁属于新增产品范围，不进入冻结的 `1.0.0-beta.1`。收费版本实现前，需要补充需求、验收标准和隐私说明。
- 已解锁用户的核心播放在离线状态下必须可用；试用和权益判断不得依赖 Vesqen 服务器。
- 与 G5 的冲突：Play Billing 9.1.0 会通过传递依赖引入 `INTERNET` 权限和 Google 的日志上报组件。含 Billing 的构建能否移除 `INTERNET`，需要真机验证；在此之前，G5 和"应用无法联网"的对外表述只适用于不含 Billing 的构建。
