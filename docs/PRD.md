...
- Material 设计下的浅色、深色和动态色主题。
- TalkBack 标签、合理触控目标和字体缩放。
- 首发至少支持英文和简体中文界面。
- 高级参数需提供简明解释，避免只有缩写。
...
2. 播放器创建系统播放会话。
3. 界面显示源文件参数和当前输出设备。
4. Audio Proof 将模式标记为 `SYSTEM MIXED` 或有证据支持的 Direct 状态。
...
1. 用户从播放页打开 Audio Proof。
2. 查看源、解码、处理、系统路由和 DAC 五层信息。
3. 用户按需打开实时指标、调整刷新率并固定关心的指标或图表。
4. 用户可以保存布局，或主动录制并导出经过隐私清理且保留可信度标签的诊断报告。
