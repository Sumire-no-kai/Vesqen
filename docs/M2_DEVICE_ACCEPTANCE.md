...
## 2026-09-08 · iQOO 重连后的短时补验

本轮只有 iQOO V2171A / Android 15（API 35），没有 Honor 或 USB DAC；用户继续暂缓长时间测试。原始输出保存在私有 `build/qa/m3-followup-20260908/`。不以模拟故障通过替代真实 USB 能力或外部 bit-perfect 证明。

- 本轮前半段生产代码未修改。Debug SHA-256 `74a858ae83d25bd6d286bedd0191a9b9c8037cab8855fb71879643ccbe3ba501`；新增两项字体用例的测试 APK SHA-256 `301ee072e8f2a4dcf8533b3b2181cb57befa7f3f6de2f23cdde08c87dbafd9aa`。两个 APK 均安装成功并核对设备哈希后才运行。
- 使用 Android 的应用语言设置切到 `en-US` 后，51 项 `VesqenAppTest` 完整 runner 通过，0 失败/跳过，187.145 s。第 9 项启动期间停留在桌面，主机显式启动同一 ComponentActivity 后继续；这是包含一次前台辅助的结果，不宣称完全无辅助。
- 新增 320 dp、1.3×/1.5× 字体的解码器名称用例，检查完整文本、实际布局行数、无溢出及占用超过 80% 卡片宽。英文两项均在上面的 51 项内通过。恢复系统语言后的中文首批 1.3× 通过，1.5× 因启动辅助与 ActivityScenario 争用发生已销毁 Activity 异常；改用预先启动宿主后，原 1.5× 用例独立通过，未改断言。首次失败原文保留。
- Profile 保持上轮哈希 `87974fcd8362ab7852198a27d178084e8a6156400fa297d5091f3fc4257b87de`。真实曲库、暂停且显示迷你播放器的三轮快滑各 48 次：570/565/574 帧，HWUI jank 全部 0%，p95 11/10/10 ms，实际 60 Hz。独立冷进程 trace 有 1/794 个 App Deadline Missed，不能据此宣布 Library 已修复。逐帧解释见工程案例 P02 补测。

- 真实无损音源与存储/遥测/设置契约完整 runner 15/15，通过时间 117.244 s；无 DAC 严格失败、Controller 重连、密集模式命令及普通扬声器切歌/seek/暂停/采样/隐私导出/释放同批 2/2，16.332 s。以上都使用上述 Debug/test 配对，不能代替真实 DAC 插拔。
- 设置完整写入和下一独立 instrumentation 进程读取/重置各 1/1；读取用例断言进程 ID 不同，不用同进程缓存冒充持久化。
- 系统字体实际设为 1.3×/1.5×，分别在 en-US/zh-CN 捕获 Library 和 Now 页面。发现英文播放器会话切换按钮被省略，中文正常；随后进行局部英文文案修复，不能把先前 51 项语义/几何通过写成所有文字都没有省略。

- 英文按钮最终改为 `Session` / `Artwork`。最后 Debug SHA-256 `ce4a3ee83d5801bfee7c9c91ba3427864d9f7b91833f94d149b4015de3ebf94d`，测试 APK `c0089f721b4d1c42a443d22675459626c6c6c80a290976235ed1d6dca3b7bb23`，安装与哈希核验完成后，英文定向 4/4（13.764 s）、中文 1/1（3.284 s）通过。新按钮用例检查两种视图的完整文字，不只检查语义存在。中间候选的测试代理指标误报已按 Compose 源码解释并修正，见案例 R08。
- 本轮总共 69 个不同设备用例分别取得通过，包含不同语言、不同候选及独立批次，不是一次 69/69 runner，也不是最终 APK 全矩阵验收。完整英文套件和音频/存储批次早于仅修改两条英文资源的候选，最后只复验受影响的界面用例；没有重跑未受影响的整套音频测试。
- 本地 Debug/Profile 与 instrumentation 构建成功；最终测试源码编译和 Lint 通过，0 errors / 21 个既有 warnings。本轮没有重新执行 JVM 全套或单独 Release 构建，不能把此前的 195 项结果算作本轮重新执行。
- 数据审计仍为 112 首、1 个来源；稳定 ID、收藏及收藏顺序、歌单一致，SQLite integrity 为 ok。已备份测试前后数据、恢复原播放 checkpoint 与浏览偏好，Chain 偏好原本不存在且保持不存在；六项系统设置及应用语言恢复。只删除本轮新增的两个测试偏好文件，以及已拉取并核对哈希的手机 trace 副本。实际测试产生的播放历史没有回滚，不宣称整个数据库逐字节未变。
- 最终 Profile SHA-256 `3b8d1f5289991a4c1d0a19381d8448783987c1a8efe3e2f83b78951d73380f7a` 已安装并核验非 debuggable。无外设实际能力、旧系统、完整无障碍/格式矩阵、Library 修复及长时门禁仍未关闭；本轮不发布、推送或合并。

最终 Profile 的 1.5× 英文实屏复查确认 `Session` 与 `Artwork` 都完整显示，并能在两个视图间切换。该截图也显示会话信息行的 `Playback progress` 标签仍有省略，时间读数完整；这项列入 M4 中英文大字体信息行适配待办，不据按钮修复关闭整个界面矩阵。

## 2026-09-15 · 冻结前的链路布局与严格输出反馈修复

- 本轮只调整 UI、资源、测试和说明；没有更改播放引擎、严格输出判定或持久化格式。链路概览先解释当前输出，再展示左对齐的源文件 / AudioTrack / 路由与稳定性数据。播放器顶部 USB 入口与 Settings 共享模式，严格失败由应用层解释原因并提供明确恢复选择，禁止自动降级。
- JDK 21：最终 209 项 JVM 测试全部通过，无失败或跳过；Debug 构建和 Lint 通过，0 errors / 18 warnings；设备测试 APK 编译通过。正式包名 Debug SHA-256：`614ddab277843889494f6442ff0507e786d42511d7b93326fb0c5e63cf1fbe44`。本轮未运行 Release、远端 CI 或长时测试，未冻结、提交或推送。
- 设备为 iQOO V2171A / Android 15。电脑调试证书与原安装不同，使用临时构建配置生成独立 `io.github.sumirenokai.vesqen.qa` 包，未覆盖原版，也未迁移其私有曲库数据。最终实际安装 QA APK SHA-256：`097f3adcbbf161975d7da4f09464d1aaffa0168a3d2b3f09659de2e1564eb44b`；配套 test APK：`1e997bf2489fa18cc2fb03ec8e24b9a48ec3630fa4b165dab2e58ad04d92d33f`。
- 最终专项 runner **10/10** 通过，35.706 s，0 失败/跳过；使用已有 vivo 前台宿主辅助，不宣称全程无辅助。覆盖失败原因、保留模式、重复失败提示、服务确认前不关闭恢复弹窗、控制器未连接时禁用恢复；播放器 320/360 dp、横屏及 200% 字体入口；360/600 dp 数字左对齐且不随位数/单位重排；130% 字体下源文件与 AudioTrack 分组堆叠、滚动后证据完整可见。
- 中间失败与修正分别保留：大字体旧用例未滚动到 Settings 的入口；使用可见区域裁切矩形比较屏幕外元素导致错误的零坐标，改为真实布局坐标并增加滚动后的可见性断言；vivo 宿主前台辅助与 ActivityScenario 发生竞争，清理 QA 残留宿主后复验通过。没有删除、跳过或降低原有布局和证据断言。
