...
- **Motion:** Opening Now follows the mini-player's 240ms shared-axis path: the focused surface rises from 16% below its final field after a short handoff, while the compact shell recedes. Returning uses a 180ms inverse path: the focused surface moves down and scales toward the mini-player, then Library, compact navigation, and the mini-player reveal in the final 108ms. This prevents the previous full-player/library double-exposure. A user-initiated previous/next change moves only artwork and track identity 220ms in the corresponding horizontal direction while the transport dock remains spatially stable. Reduced motion replaces these movements with the 80ms crossfade fallback.
